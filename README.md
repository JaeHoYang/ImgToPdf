...
> Windows MFC 기반 다기능 파일 변환 데스크톱 앱 — 이미지 ↔ PDF · PDF 도구 · Markdown · Word · PPT 변환
...
5개의 탭으로 이미지·PDF·Markdown·Word·PPT 변환을 하나의 창에서 처리하며, 한국어/영어 런타임 전환을 지원합니다.  
...
├────────────────────────────────────────────────────────────────────────────────────┤
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  [◀] [▶]  │
│  │이미지변환│  │ PDF 도구 │  │ MD 변환  │  │Word 변환 │  │ PPT 변환 │            │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘  └──────────┘            │
│ ┌──────────────────────────────────────────────────────────────────────────────┐   │
│ │                        활성 탭 자식 다이얼로그 영역                          │   │
│ └──────────────────────────────────────────────────────────────────────────────┘   │
...
| 기능 | 설명 |
|------|------|
| Word → PDF | `.doc` / `.docx` → PDF 변환 |
| 변환 엔진 자동 감지 | Microsoft Word → LibreOffice 순으로 레지스트리 탐색, 설치된 엔진 자동 선택 |
| Microsoft Word | COM IDispatch로 `Presentations.Open` → `SaveAs(PDF)` → `Close` |
| LibreOffice | `soffice.exe --headless --convert-to pdf` 프로세스 실행 |
| 드래그 앤 드롭 | 파일 드롭 지원 |
| 미리보기 | 목록 클릭 시 쉘 썸네일 또는 파일 아이콘 표시 |

### 탭 5 — PPT 변환

| 기능 | 설명 |
|------|------|
| PPT → PDF | `.ppt` / `.pptx` → PDF 변환 |
| 변환 엔진 자동 감지 | Microsoft PowerPoint → LibreOffice 순으로 레지스트리 탐색 |
| Microsoft PowerPoint | COM IDispatch로 `SaveAs(ppSaveAsPDF=32)` 사용, 원본 슬라이드 크기 유지 |
| LibreOffice | `soffice.exe --headless --convert-to pdf` 프로세스 실행 |
| 드래그 앤 드롭 | 파일 드롭 지원 |
| 미리보기 | 목록 클릭 시 쉘 썸네일 또는 파일 아이콘 표시 |
...
1. [Releases](https://github.com/JaeHoYang/ImgToPdf/releases) 페이지에서 `ImgToPdf_Setup_v2.3.exe` 다운로드
2. 실행 후 설치 마법사 진행
...
4. `installer\ImgToPdf_Setup_v2.3.exe` 생성
...
> Microsoft Word 또는 LibreOffice 중 하나가 설치되어 있어야 합니다.

1. `.doc` / `.docx` 파일을 드래그 앤 드롭하거나 **[찾기]** 로 추가
2. *(선택)* 출력 폴더 지정 (비우면 원본 폴더에 저장)
3. **[변환]** 클릭 → PDF 저장

### PPT 변환 (탭 5)

> Microsoft PowerPoint 또는 LibreOffice 중 하나가 설치되어 있어야 합니다.

1. `.ppt` / `.pptx` 파일을 드래그 앤 드롭하거나 **[찾기]** 로 추가
2. *(선택)* 출력 폴더 지정 (비우면 원본 폴더에 저장)
3. **[변환]** 클릭 → PDF 저장
