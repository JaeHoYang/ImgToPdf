...
/**
 * @brief 탭5 — PowerPoint 파일(.pptx/.ppt)을 PDF 로 변환하는 다이얼로그.
 *
 * ### 변환 엔진 감지 순서
 * 1. **Microsoft PowerPoint** — `FindPowerPointExe()`: HKCU/HKLM 레지스트리에서
 *    POWERPNT.EXE 경로 탐색. COM IDispatch 를 통해 `PowerPoint.Application` →
 *    `Presentations.Open` → `SaveAs(ppSaveAsPDF=32)` → `Close` → `Application.Quit`
 *    순으로 처리. 출력 파일 크기는 원본 슬라이드 크기를 그대로 사용한다.
 ...
 * 각 항목 완료 시 WM_PPT_ITEM_DONE(WM_USER+25), 전체 완료 시
 * WM_PPT_CONVERT_DONE(WM_USER+26) 을 PostMessage 로 UI 스레드에 전달한다.
 *
 * ### 미리보기
 * CImagePreviewCtrl 로 선택 항목의 쉘 썸네일 또는 파일 아이콘을 표시한다.
 */
class CPptConvertDlg : public CTabDlgBase
{
public:
    /**
     * @brief 생성자.
     ...
     */
    explicit CPptConvertDlg(CWnd* pParent = nullptr);

    /** @brief 탭5 다이얼로그 리소스 ID. */
    enum { IDD = IDD_TAB5 };

    /**
     * @brief PowerPoint 파일 경로 목록을 목록에 추가한다.
     *
     * .pptx / .ppt 확장자만 허용하고 중복 경로는 건너뛴다.
     * 각 항목에 대해 FindPowerPointExe / FindLibreOfficeExe 결과를 engine 필드에 기록한다.
     *
     * @param paths 추가할 파일 경로 목록
     */
    void AddPptFiles(const std::vector<CString>& paths);

    /**
     * @brief 호스트가 WM_PPT_CONVERT_DONE 을 수신했을 때 호출한다.
     *
     ...
     *
     * 탭5는 합치기 미지원이므로 checkMerge 는 숨긴다.
     *
    ...
    /** @return 항상 false — 탭5는 합치기 체크박스를 숨긴다. */
...
protected:
    ...
    /**
     * @brief 변환 대기열의 PPT 항목 하나를 나타내는 구조체.
     */
    struct PptEntry
    {
        CString path;           ///< 파일 전체 경로
        CString name;           ///< 파일명 (표시용)
        CString engine;         ///< 사용할 변환 엔진 이름 ("PowerPoint" / "LibreOffice" / "없음")
        CString status;         ///< 변환 상태 문자열 (대기/변환 중/완료/실패)
        bool    done   = false; ///< 변환 완료 여부
        bool    failed = false; ///< 변환 실패 여부
    };
    std::vector<PptEntry> m_entries; ///< 변환 대기 항목 목록

    CListCtrl         m_listPpt;         ///< PPT 파일 목록 컨트롤
    ...
    afx_msg void OnNMClickListPpt(NMHDR* pNMHDR, LRESULT* pResult);

    /** @brief 목록 항목 우클릭 시 컨텍스트 메뉴를 표시한다 (삭제 등). */
    afx_msg void OnNMRClickListPpt(NMHDR* pNMHDR, LRESULT* pResult);

    /** @brief 목록 항목 더블클릭 시 파일을 외부 앱으로 연다. */
    afx_msg void OnNMDblclkListPpt(NMHDR* pNMHDR, LRESULT* pResult);

    /** @brief 목록 선택 변경 시 버튼 상태와 미리보기를 갱신한다. */
    afx_msg void OnLvnItemChangedListPpt(NMHDR* pNMHDR, LRESULT* pResult);

    /**
     * @brief WM_PPT_ITEM_DONE 수신 핸들러.
     *
     * wParam=항목 인덱스, lParam=성공 여부(1/0).
     * NotifyItemDone(wParam, lParam != 0) 을 호출한다.
     */
    afx_msg LRESULT OnPptItemDone(WPARAM wParam, LPARAM lParam);
    ...
    /**
     * @brief 호스트가 WM_PPT_ITEM_DONE 을 수신했을 때 내부 처리를 수행한다.
     *
     * 해당 항목의 done/failed 플래그와 상태 문자열을 갱신하고 리스트 행을 업데이트한다.
     *
